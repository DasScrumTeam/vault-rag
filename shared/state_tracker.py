...
from pymerkle import InmemoryTree as MerkleTree  # type: ignore
...
class StateTracker:
    ...
    def save_state(self, tree: MerkleTree, manifest: Dict[str, str]) -> None:
        """
        ...
        logger.info(
            "Saved new state with root hash "
            f"{state['root_hash']} to {self.state_file_path}"
        )
    ...
    def load_state(self) -> Tuple[Optional[str], Dict[str, str]]:
        ...
        if not os.path.exists(self.state_file_path):
            logger.warning(
                f"State file not found at {self.state_file_path}. Assuming first run."
            )
            return None, {}
        ...
        try:
            with open(self.state_file_path, "r", encoding="utf-8") as f:
                state = json.load(f)
            logger.info(
                f"Loaded state with root hash "
                f"{state.get('root_hash')} from {self.state_file_path}"
            )
            return state.get("root_hash"), state.get("manifest", {})
        except (json.JSONDecodeError, KeyError) as e:
            logger.error(
                "Failed to load or parse state file at "
                f"{self.state_file_path}: {e}. Treating as first run."
            )
            return None, {}

    def compare_states(
        self, old_manifest: Dict[str, str], new_manifest: Dict[str, str]
    ) -> Dict[str, List[str]]:
        """
        Compares two file manifests to determine which files were added, updated, or
        removed.
        ...
        Returns:
            A dictionary with three keys: 'added', 'updated', and 'removed', each
